...
export function PerformanceCard({ performance, onOpen }: { performance: Performance; onOpen: () => void }) {
  return (
    <article onClick={onOpen} className="flex cursor-pointer gap-4 rounded-lg border border-line bg-surface p-4">
      {performance.posterImageUrl
        ? <img src={performance.posterImageUrl} alt="" className="h-24 w-16 flex-shrink-0 rounded object-cover" />
        : <div className="flex h-24 w-16 flex-shrink-0 items-center justify-center rounded bg-surface-muted text-center text-[10px] text-ink-muted">포스터 없음</div>}

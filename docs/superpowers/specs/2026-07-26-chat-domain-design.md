...
Attaca의 마지막 미구현 도메인 CHAT을 **BE 먼저** 구현한다. 1:1(DIRECT)과 그룹(GROUP) 실시간 채팅을 하나의 방 모델로 통합하고, 메시지 전송/수신/이력/방목록 + 읽음(안 읽은 수) + 접속 상태(presence) + 입력 중(typing)까지 다룬다. FE 화면은 다음 이터레이션.

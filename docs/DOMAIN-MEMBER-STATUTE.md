...
* `POST /api/auth/oauth/kakao` : 카카오 소셜 로그인. body `{code, redirectUri}` → 기존 완료 회원은 `TokenPairResponse`, 신규 또는 온보딩 미완료 회원은 짧은 수명의 `onboardingTicket`과 `isNewMember: true`를 반환한다. 프론트가 카카오에서 받은 1회용 인가코드를 전달하면 백엔드가 교환한다.
...
* 기존 완료 회원과 자체 로그인은 `JwtProvider` access+refresh 발급으로 수렴한다. 신규 소셜 회원은 먼저 `purpose=SOCIAL_ONBOARDING` 온보딩 티켓을 받고, 닉네임·필수 동의 완료 뒤 정식 토큰으로 교환한다.
...
### 3.2.1 회원 검색 (2026-09-09 도입)

CHAT이 1:1 대화를 시작할 때 상대를 고르려면 닉네임으로 사람을 찾을 수 있어야 한다.
그전에는 화면이 **회원 id를 숫자로 입력받고** 있었는데, 그건 사용자가 알 수 있는 값이 아니다.

* `GET /api/members/search?q=&size=` (인증) → `MemberDisplay[]` (`{id, nickname, verified}`)
* **회원 목록을 여는 일**이므로 아래를 규칙으로 못박는다.
  * **인증 필수.** 비로그인에게는 열지 않는다(공개 조회 경로 `/api/public/**`에 두지 않는다).
  * 응답은 `MemberDisplay`뿐이다. 이메일·로그인 id·가입일 등 **식별·연락 정보는 절대 싣지 않는다.**
  * `q`는 **공백 제거 후 2자 이상**이어야 한다. 1자로 전체 회원을 훑는 것을 막는다. 미만이면 빈 목록(400이 아니다 — 타이핑 중이 정상 상태다).
  * 부분 일치(대소문자 무시). 접두 일치만 하면 "정하윤"을 "하윤"으로 못 찾는다.
  * `size` 기본 10 / 최대 20. 페이징을 주지 않는다 — 더 보려면 더 정확히 입력하게 한다.
  * **본인은 결과에서 제외**한다(자기 자신과 대화할 수 없다).
  * **탈퇴 회원은 제외**한다(`deletedAt is null`). 탈퇴 시 닉네임이 `탈퇴한회원{id}`로 바뀌므로 검색에 걸릴 수 있다.
* 정렬은 닉네임 오름차순. 관련도 정렬은 두지 않는다(결과가 최대 20건이라 의미가 없다).
...
* **가입 완료 시 동의가 없으면 거절한다.** 자체 가입은 가입 요청에서, 신규 소셜 가입은 온보딩 티켓을 사용한 닉네임 설정 요청에서 둘 다 확인한다. `CONSENT_REQUIRED`(400-04).
...
### 3.5 비밀번호 변경 (2026-09-09 도입)

**바꿀 방법이 없었다.** 회원이 스스로 할 수 있는 것은 프로필 수정과 탈퇴뿐이라,
비밀번호가 새면 **탈퇴 말고는 손쓸 방법이 없었다.**

* `PUT /api/members/me/password` (인증). body `{currentPassword, newPassword}`
  → `TokenPairResponse`.
* **현재 비밀번호를 확인한다.** 인증된 요청이어도 다시 묻는다 — 자리를 비운 사이
  남이 쥔 화면으로 비밀번호가 바뀌면 계정을 통째로 빼앗긴다.
* 새 비밀번호는 가입과 **같은 규칙**을 따른다(§3.3). 규칙이 갈리면 어느 한쪽이 약해진다.
* **같은 값으로는 바꿀 수 없다.** 바꾼 줄 알았는데 그대로인 상태를 만들지 않는다.
* **소셜 전용 회원은 바꿀 수 없다.** `password`가 null이라 확인할 현재 값이 없다.
  자체 로그인을 새로 여는 것은 다른 기능이므로 여기서 하지 않는다.
* **성공하면 refresh를 전부 철회한다.** 비밀번호를 바꾸는 이유의 절반이
  "남이 들어와 있을지 모른다"이므로, 다른 기기를 끊지 않으면 바꾸나 마나다.
  * 다만 **부른 본인에게는 새 토큰 쌍을 발급해 돌려준다.** 전부 끊고 끝내면
    비밀번호를 바꾼 사람이 자기도 튕겨 나가 다시 로그인해야 한다.
  * 이미 발급된 access(30분)는 만료까지 산다 — 로그아웃·탈퇴와 같은 절충이다.
* 에러코드: `CURRENT_PASSWORD_MISMATCH`(400-05), `PASSWORD_UNCHANGED`(400-06),
  `PASSWORD_NOT_SET`(400-07)
* 확인 입력(새 비밀번호 재입력)은 **화면에서만** 다룬다. 서버로 보내지 않는다.

**⚠️ 알려진 상호작용 — 재사용 감지와 겹친다.** 철회된 옛 refresh를 다른 기기가
들고 있다가 갱신을 시도하면, 화이트리스트에 없으므로 **탈취로 간주돼 전 기기가
날아간다**(§`DOMAIN-COMMON-STATUTE` §4.1). 그러면 비밀번호를 방금 바꾼 본인의
새 세션까지 끊긴다.

우리가 **일부러 철회한** 토큰이 도는 것은 탈취의 증거가 아니라 예상된 일인데,
지금 구조는 그 둘을 구분하지 못한다. 보안이 깨지는 것은 아니고(다시 로그인하면 된다)
"바꿔도 로그인 유지" 의도만 어긋난다. 고치려면 회원별 철회 시각을 두고
그보다 먼저 발급된 토큰은 **전 기기 무효화 없이** 거절해야 한다.
`docs/TODO-BACKLOG.md` 참고.

> 비밀번호 **재설정**(잊어버렸을 때)은 아직 없다. 메일 발송 수단이 선행 조건이라
> 따로 다룬다. `docs/TODO-BACKLOG.md` 참고.

### 3.6 회원 탈퇴 (2026-09-09 도입)

...
* [x] ~~FE 구인: 악기 표시가 enum명 그대로~~ — 2026-09-09 해소. `lib/recruitment/instruments.ts`(`toLabelMap`/`instrumentText`) 신설. 옵션은 화면이 이미 받아오고 있었고 카드에 안 내려주던 것뿐이라, 목록은 prop으로 전달하고 상세는 직접 조회한다. 모르는 코드는 **코드 그대로** 남긴다(빈칸이 되면 파트 없는 공고처럼 보인다). 원문:  — profile-options의 label 맵을 넘겨 한글 라벨로 변환. 목록 성능 위해 이번엔 단순화.
...
* [x] ~~FE 구인: 지원 실패 에러가 화면 밖~~ — 2026-09-09 해소. 에러를 액션 지점(지원 패널/지원자 목록) 바로 위로 옮기고 `role="alert"` 부여. 원문: (`<dl>` 위)에 뜨는데 `ApplyPanel`은 하단이라 스크롤해야 보임 — 에러를 액션 지점(패널) 근처에 렌더하거나 토스트로. (최종 리뷰 Minor)
* [x] ~~FE 구인: 마감/수락/거절 연타 가드~~ — 2026-09-09 해소. 단일 `pending` 키로 진행 중 액션을 표시하고 버튼을 비활성화(마감은 '마감 중...'). 응답을 붙잡아 둔 채 연타해 요청이 한 번만 나가는 것을 테스트로 확인. 원문: (지원 제출만 submitting으로 막힘) — 연타 시 중복 요청 가능. (최종 리뷰 Minor)
...
* [x] ~~FE 인증연주자: 증빙 링크 URL 형식 검증~~ — 항목이 낡았다. FE `validateApply`는 이미 `isHttpUrl`로 검증하고 있었고, 2026-09-09에 **BE `ApplyRequest`에도 스킴 검증을 추가**해 저장 자체를 막는다(티켓 링크 XSS 수정과 같은 작업).
...
* [x] ~~**외부 공연 정보(KOPIS)·입시 모집요강 반입**~~ — 2026-09-14 구현 완료. 사양 승인·도메인 문서·구현 계획·BE/FE·운영 문서 반영 완료. 실제 KOPIS 호출과 대학 게시판 운영 smoke test는 인증키·연락처 설정 후 진행한다.
  * 법적 검토 결론(2026-09-11): 티켓 판매처(인터파크·예스24·멜론티켓) 크롤링은 약관상 금지라 **하지 않는다**. 공연은 **KOPIS(공연예술통합전산망) Open API**로 받는다(무료·출처 표기 의무·포스터는 URL 링크만). 입시 모집요강은 PDF 원문 저장·재배포 대신 **사실 정보(학과·전형·접수기간·실기곡목) + 입학처 원문 링크**로 다룬다(사립대 요강은 저작물, 국공립은 공공누리 표시 있을 때만 자유이용).
  * 운영 규칙(구현 시 STATUTE로 승격): robots.txt·요청 간격·식별 UA, 로그인 우회 금지, 저작물 원문 미저장, 출처·수집 시각 표시, 특정 사이트 통째 미러링 금지.
  * 확정(2026-09-11 브레인스토밍): **자동 수집 → 어드민 승인 → 공지(NOTICE)로 승격.** 별도 도메인 IMPORT 신설, NOTICE에는 출처·원문 링크 필드 추가. 승인 시 공지 폼이 채워진 채 열린다. 수집은 BE 주 1회 스케줄 + 어드민 "지금 가져오기".
  * KOPIS: 클래식(`shcate=CCCA`) 전국, 향후 8주. 포스터는 승인 시에만 내려받아 FileService에 저장(공개 화면 핫링크 금지, 어드민 심사 썸네일만 원본 URL).
  * 입시: 공공 전형 API가 없음을 확인(대학어디가는 API 없음·약관상 DB 재활용 금지). **대학 입학처·음대 공지 목록의 제목·링크·날짜만** 수집, 본문·PDF 미저장. 2026-09-13 대학 17곳의 음악 전공·robots·약관·렌더링·인코딩·글 번호 규칙 조사를 완료했으며, 사양서 §10의 확인 완료·설정 후보만 구현 설정에 넣는다.
...
* [x] ~~**어드민 부트스트랩**~~ — 2026-09-09 완료. 환경변수 `ADMIN_LOGIN_IDS` 로 기동 시 승격. 올리기만 하고 내리지 않으며, 없는 loginId는 경고만 남기고 기동을 막지 않는다. 규칙은 DOMAIN-MEMBER-STATUTE §4.1.
  * 어드민이 다른 회원을 승격시키는 API는 아직 없다 — 두 번째 어드민도 이 환경변수로 만든다. 필요해지면 그때 만든다.
...
* [ ] **비밀번호 재설정, 이메일·닉네임 변경이 없다** (2026-09-09 확인, 같은 날 비밀번호 *변경*은 도입). 회원이 스스로 바꿀 수 있는 것은 프로필(악기·소개·사진)·비밀번호·탈퇴뿐이다. **닉네임은 `UpdateProfileRequest`에 아예 없어서 요청에 넣어도 조용히 무시된다** — 서비스 전역 표시명인데 오타를 고칠 수단이 없다. 유니크 제약이 있어 중복 처리와 기존 글 표시(배치 조회라 자동 반영) 확인이 함께 필요하다.
  * 비밀번호가 새면 **탈퇴 말고는 손쓸 방법이 없다.** 잊어버렸을 때 되찾을 방법도 없다.
  * 이메일 변경이 없어서 **계정을 다른 사람에게 넘길 수 없다** — 시드 계정을 당사자에게 이관할 때 실제로 걸렸다(`scripts/seed/README.md` 참고).
  * 필요한 것: `PUT /api/members/me/password`(현재 비밀번호 확인 후 변경, 변경 시 refresh 전부 철회), 이메일 변경(소셜 자동연결 매칭 키라 신중히), 비밀번호 재설정(메일 발송 수단이 선행 조건).

* [ ] 업로드 동작을 배포 후 확인하는 수단 — 2026-09-09에 운영에서 파일 업로드가 통째로 실패하고 있었는데(볼륨 권한) 아무도 올린 적이 없어 몰랐다. `/actuator/health`는 통과했다. 쓰기 가능 여부를 헬스체크에 넣거나, 배포 후 확인 목록에 업로드를 넣을 것.
...
* [ ] **의도한 철회와 재사용 감지를 구분한다** (2026-09-09 확인). 비밀번호 변경·전 기기 로그아웃처럼 **우리가 일부러 철회한** 뒤에는 옛 refresh가 도는 것이 예상된 일인데, 지금은 화이트리스트에 없다는 이유로 탈취로 간주해 전 기기를 날린다. 그러면 비밀번호를 방금 바꾼 본인의 새 세션까지 끊긴다.
  * 보안이 깨지는 것은 아니다(다시 로그인하면 된다). "바꿔도 로그인 유지" 의도만 어긋난다.
  * 고치는 방법: 회원별 철회 시각(`tokensRevokedAt`)을 두고, 그보다 먼저 발급된 refresh는 **전 기기 무효화 없이** `REVOKED_TOKEN` 으로 거절한다. 그 이후 발급된 것이 화이트리스트에 없으면 그때는 진짜 재사용이다.

* [ ] 공지 커버 이미지 업로드 화면 — BFF 라우트(`PUT /api/bff/admin/notices/{id}/cover`)는 뒀는데 화면이 없다. 커버 없는 공지는 캐러셀에서 "이미지 없음"으로 나온다.
...
* [ ] 홈 후속: 캐러셀 자동 전환을 **hover·포커스 시** 일시정지. *(2026-09-09에 «직접 넘기면 정지»와 `prefers-reduced-motion`(motion-reduce)은 이미 도입 — 남은 것은 마우스를 올리거나 포커스가 들어왔을 때의 일시정지뿐이다.)*
* [ ] 홈 후속: 공지 상세 화면(`/notices/[id]`)이 없어 달력의 공지 항목은 클릭할 수 없다. 캐러셀의 공지 슬라이드도 CTA가 없다.
* [x] ~~홈 후속: 히어로의 "이미지 없음" 회색 박스~~ — 2026-09-09 해소. 사진 없는 장은 어둠막 없이 글을 세로 가운데 두는 타이포 중심 레이아웃으로 분기했다. 원문: 히어로 이미지가 없을 때 "이미지 없음" 회색 박스가 그대로 보인다 — 포스터 없는 공연이 많으면 밋밋하다. 타이포 기반 대체 디자인 검토.

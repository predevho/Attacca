...
* [ ] **비로그인 방문자가 공연을 볼 수 없다** (2026-09-09 확인). 공개 API(`/api/public/performances`, scope PAST/ALL)는 있는데 `/performances` 화면이 미들웨어 보호 라우트라 로그인으로 튕긴다. 홈 캐러셀은 UPCOMING만 보므로 지난 공연은 어디에도 안 보인다 — 시드를 넣어도 방문자 화면은 그대로 비어 있다.
  * 포폴 데모로서 치명적이다. 링크를 받은 사람이 처음 보는 화면이 빈 홈이다.
  * 후보: (1) `/performances` 목록·상세를 공개로 열고 쓰기 동작만 로그인 요구 (공개 API가 이미 있으므로 자연스럽다) (2) 홈에 '지난 공연' 구획 추가 (3) 다가오는 공연 데이터 확보.
  * 라우팅 공개 범위는 제품 결정이라 임의로 바꾸지 않고 남긴다.

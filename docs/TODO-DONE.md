...
* [x] (2026-09-17) 회원가입 아이디·비밀번호 길이 규칙 조정.
  ...
  * 로그아웃 버튼의 포인터 커서 피드백은 구현하지 않고 `TODO-BACKLOG`에 UX 개선 항목으로 기록했다.

* [x] (2026-09-16) 로그인·소셜 온보딩 회귀 수정과 피드 조회 기본 인덱스 보강.
  * 로그인 화면의 약관 체크를 제거하고, 약관 동의는 신규 소셜 회원의 닉네임 설정 단계에서만 받는다. 비밀번호 재확인에는 입력 중 일치/불일치 안내를 표시한다.
  * 로그인 성공은 기본 홈(`/`)으로 이동하고, 로그아웃은 헤더 상태를 지운 뒤 `/login`으로 이동한다. 카카오 버튼은 항상 인가를 시작하며, 설정이 없을 때만 설정 오류를 보여 준다.
  * 소셜 온보딩을 끝내지 않고 재로그인한 회원도 닉네임 설정으로 돌아가도록 `isNewMember`를 온보딩 필요 여부로 해석한다. 완료 전 인증 API 차단은 서버에서 유지한다.
  * 피드 타임라인용 게시물·좋아요·댓글 인덱스를 Flyway V6로 추가했다. 실제 운영 부하 측정은 별도 관측 작업으로 남긴다.

* [x] (2026-09-09) 어드민 공지 관리 화면 — **NOTICE 도메인의 빠진 절반**. 규칙은 `docs/DOMAIN-NOTICE-STATUTE.md` §8.
  * **BE는 처음부터 CRUD 5종이 있었는데 FE에 BFF 라우트도 화면도 없었다.** `TODO-DONE` 에 "NOTICE 도메인 문서화 + BE 구현"으로만 적혀 있었고, FE는 공개 읽기(홈 캐러셀·달력)만 있었다. 즉 **공지를 올릴 방법이 아예 없었다** — 홈 캐러셀과 달력의 원천이 공지인데도.
  * 로컬에서 화면을 한 바퀴 돌다 `/api/bff/admin/notices` 404로 드러났다.
  * `/admin/notices`: 목록·등록·수정·삭제. 등록과 수정이 같은 폼을 쓴다.
  * **일정(EVENT)인데 일시가 없으면 막는다** — `scheduledAt` 이 달력 노출의 유일한 상태라 일시 없는 일정은 올린 뜻이 사라진다.
  * 헤더의 어드민 링크를 '공지'·'인증심사' 둘로 나눴다.
  * **검증(실화면)**: 공지 3건 목록·고정 배지 표시 → 일정을 일시 없이 제출하면 차단 → 일시를 넣어 등록 → **홈 달력 20일에 점이 찍히고 '이번 달 일정'에 표시**. 달력이 처음으로 채워졌다.
  * 남긴 것: 커버 이미지 업로드는 BFF 라우트만 두고 화면은 안 붙였다.

* [x] (2026-09-09) 비밀번호 변경 — `PUT /api/members/me/password`. 규칙은 `docs/DOMAIN-MEMBER-STATUTE.md` §3.5.
  * **없어서 생긴 문제**: 바꿀 방법이 없어 비밀번호가 새거나 잊으면 **탈퇴 말고는 손쓸 방법이 없었다.**
  * 인증된 요청이어도 **현재 비밀번호를 다시 묻는다** — 자리를 비운 사이 남이 쥔 화면으로 바뀌면 계정을 통째로 빼앗긴다. 새 비밀번호는 가입과 **같은 규칙**(규칙이 갈리면 한쪽이 약해진다). 같은 값 거절. 소셜 전용 회원은 확인할 현재 값이 없어 거절.
  * **성공하면 refresh를 전부 철회**하되 **부른 본인에게는 새 토큰 쌍을 준다.** 전부 끊고 끝내면 비밀번호를 바꾼 사람이 자기도 튕겨 나간다.
  * **검증(로컬 실기동 + 실화면)**: 두 기기 로그인 → 기기1에서 변경 → 기기2 refresh 401-09, 기기1 옛 refresh 401-09, **기기1 새 refresh 성공**, 옛 비밀번호 로그인 401-07, 새 비밀번호 성공. 화면에서도 같은 값·확인 불일치가 막히고 성공 후 로그인이 유지되는 것을 확인.
  * **알려진 상호작용**: 다른 기기가 옛 refresh로 갱신을 시도하면 재사용 감지가 전 기기를 날려 본인 새 세션까지 끊긴다. 백로그에 기록.
  * **작업 중 사고**: 문서 절 번호 일괄 치환(§3.5→§3.6)이 **적용된 Flyway 마이그레이션의 주석까지** 바꿔 로컬 BE가 체크섬 불일치로 죽었다. 되돌려 복구했고, 재발 방지로 `db/migration/README.md` 를 뒀다(마이그레이션 파일에는 바뀔 수 있는 참조를 적지 않는다).

* [x] (2026-09-09) 어드민 부트스트랩 — 첫 어드민을 코드로 만들 수 있게 했다. 규칙은 `docs/DOMAIN-MEMBER-STATUTE.md` §4.1.
  * **없어서 생긴 문제**: 코드에 ADMIN 부여 경로가 전혀 없어 DB를 직접 고쳐야 했고, 그러지 않으면 배포된 서비스는 영영 공지를 못 올리고 인증 연주자를 승인할 수 없었다.
  * 환경변수 `ADMIN_LOGIN_IDS`(쉼표 구분)로 기동 시 승격한다.
  * **올리기만 하고 내리지 않는다** — 설정 한 줄로 운영자가 조용히 사라지면 그게 더 위험하다. **계정을 만들지 않는다** — 없는 loginId는 경고만 남기고 기동을 막지 않는다(어드민 하나 때문에 서비스가 안 뜨는 게 더 나쁘다). **모든 승격을 로그로 남긴다.**
  * **`@Transactional` 위치가 함정이었다.** `run(ApplicationArguments)` 안에서 내부 메서드를 부르면 자기 호출이라 프록시를 거치지 않아 트랜잭션이 열리지 않고, 더티 체킹으로 올린 role이 flush되지 않는다 — **조용히 아무 일도 일어나지 않는다.** 단위 테스트(@DataJpaTest)는 자체 트랜잭션 안에서 돌아 이걸 못 잡는다. 실제 기동으로 DB 반영까지 확인해서 알았다.
  * **검증**: 로컬에서 `ADMIN_LOGIN_IDS="localadmin, ,notyetsignedup"` 로 띄워 승격 로그 + DB 반영 확인, 없는 loginId는 경고만, 빈 항목은 무시, 재기동 시 로그 0줄(멱등). BE 417 통과.

* [x] (2026-09-09) 로컬 환경 복구 + 화면 전체 점검. **여기서 버그 2건을 잡았다.**
  * 그동안 검증을 전부 운영에 대고 했다(가짜 계정 생성·삭제, 탈퇴 시험, DB 직접 수정). 로컬을 띄워 앞으로는 여기서 먼저 확인한다. 운영에서만 확인할 것은 인프라 성격의 것들(nginx·HTTPS·볼륨 권한·마이그레이션·자동 배포)로 한정한다.
  * **`FE/.env.local` 이 8081을 보고 있었다** — 예전에 다른 프로젝트가 8080을 점유해 옮겨 둔 값이 남아, BFF가 BE에 붙지 못했다. 8080으로 되돌리고 이유를 주석에 남겼다.
  * **탈퇴 후 헤더가 갱신되지 않았다.** `router.push('/')` 로 나가면 클라이언트 상태가 살아 있어, 방금 계정을 지운 사람에게 '로그아웃' 메뉴가 계속 보였다. 헤더는 신원을 마운트 때 한 번만 읽고 경로가 바뀌어도 다시 읽지 않으며, `router.refresh()` 는 서버 컴포넌트만 새로 그려 이 상태를 지우지 못한다. 전체 새로고침으로 나가게 고쳤다. 로그아웃은 헤더가 숨겨지는 `/login` 을 거쳐 우연히 멀쩡했다.
  * **점검한 흐름**: 회원가입(검증 6종 화면에서 차단 → 정상 가입) → 로그인 → 글쓰기 → 홈 위젯 반영 → 공연 목록·상세·포스터 → 탈퇴(확인 문구 가드 포함). 어드민 API(인증 부여·공지 등록)도 처음으로 실제 호출해 확인했다.
  * 로컬 시드에는 공지 3건까지 넣어 캐러셀이 고정 공지 1 + 지난 공연 3으로 도는 것을 확인했다.

* [x] (2026-09-09) 홈 빈 상태 개선 — 히어로가 사라지지 않게 한다.
  * 다가오는 공연이 없으면 히어로가 **통째로 사라져** 홈 위쪽이 텅 비었다(`if (!current) return null`). 방문자가 처음 보는 화면이 그거였다.
  * `toSlides`가 다가오는 공연이 하나도 없을 때만 지난 공연을 뒤에 덧붙인다. 배지는 **'지난 공연'으로 따로** 단다 — `PERFORMANCE`로 뭉뚱그리면 지난 것이 다가오는 것처럼 읽힌다.
  * 태울 것이 정말 없으면 `EmptyHero` 카드를 놓는다(빈 DB·첫 배포용).
  * 빈 문구에 다음 걸음을 붙였다: 게시글 → '첫 글 남기기', 이번 달 일정 → '공연 둘러보기'. 이번 달이 비어도 지난 공연은 있다.
  * **검증(실브라우저, 비로그인)**: 히어로에 '지난 공연' 배지와 포스터가 뜨고 3장이 순환. 빈 상태 링크 모두 표시.

* [x] (2026-09-09) 공연 목록·상세를 공개로 전환 — **보기는 공개, 쓰기는 로그인**.
  * 공개 API(`/api/public/performances`)가 이미 있는데 화면이 통째로 보호 라우트라, 링크를 받은 사람이 공연을 하나도 볼 수 없었다. 시드를 넣어도 방문자 화면은 빈 홈 그대로였다.
  * 미들웨어 matcher를 `/performances/:path*` → `/performances/new`, `/performances/:id/edit` 로 좁혔다.
  * 목록 화면이 신원 조회 실패 시 로그인으로 튕기던 것을 없앴다 — 이제 '등록' 버튼만 감춘다. **여기가 실제 막힘의 절반이었다**(미들웨어만 열어도 화면이 스스로 튕겼다).
  * 상세는 로그인 여부로 경로를 나눈다. 공개 응답에는 주최자의 회원 id가 없어(`PublicMemberDisplay`) 본인이어도 수정·삭제 버튼을 띄울 수 없기 때문이다.
  * 타입에 반영: `organizer.id`·`updatedAt` 선택. `canEdit`/`canDelete` 가 id 없음을 명시적으로 다룬다 — 대상이 누구인지 모르면 아무도 수정할 수 없는 것이 맞다.
  * **검증(실브라우저, 비로그인)**: `/performances` 200 → '지난' 탭에 공연 4건 표시 → 상세에 포스터·프로그램 렌더 → 수정·삭제 버튼 없음. `/performances/new` 와 `/feed` 는 로그인으로 이동.

* [x] (2026-09-09) 시드 데이터 투입 — 공연 4건. 절차는 `scripts/seed/README.md`.
  * 실제 공연 자료(`resource/`)에서 옮겼고 **지난 공연은 실제 날짜 그대로** 넣었다. 정음피아노앙상블 제4회 정기연주회(2024-06-24 일신홀) / 모차르트의 밤(2024-06-28 백남음악관) / TWO PIANO OPERA(2025-02-15 뮤즈아트홀) / 제40회 영아티스트 콘서트(2024-02-17 영산아트홀). 곡목은 각 프로그램 그대로.
  * **계정은 실명이 아니라 `공연 아카이브`**(중립적인 이름, id=7). 계정을 만드는 순간 이름과 얼굴이 공개 사이트에 올라가고 동의 기록이 그 이름으로 남는데, 본인 확인 없이 그렇게 하지 않기로 했다. 당사자가 직접 쓰겠다고 하면 본인이 가입한 뒤 공연을 옮긴다(README '데이터 이관' — `organizerId`가 원시 Long이라 UPDATE 한 줄).
  * **어드민 계정을 만들지 않았다.** 시드를 돌리자고 권한이 큰 계정을 늘리지 않는다. 인증 부여는 기존 어드민 명의로 한 번, 공지는 문안만 `data.json`에 남기고 화면에서 쓰기로 했다.
  * **제외한 이미지**: 영아티스트 콘서트 포스터(미성년자 13명 이상 얼굴, 주최도 다른 단체), 정음 프로그램 뒷면(연주자 11명 얼굴). 공연 정보·곡목은 공개 정보라 쓰되 사진과 개인정보는 다르게 다뤘다.
  * **함께 잡은 운영 버그**: `/app/uploads` 볼륨이 root 소유라 **파일 업로드가 통째로 실패**하고 있었다(uid 10001로 도는데 쓰기 불가, 500 `FILE_UPLOAD_FAILED`). 포스터·프로필 사진·공지 커버 전부 해당. 아무도 올린 적이 없어 가려져 있었고 `/actuator/health`도 통과했다. Dockerfile에서 디렉터리를 미리 만들어 소유자를 넘기고, 기존 볼륨은 한 번 chown 했다.
  * **검증**: 공개 목록 4건, 포스터 3장이 `https://attacca.site/files/...` 로 200 image/jpeg 응답.
  * **남은 것**: 비로그인 방문자는 아직 이 데이터를 볼 수 없다(아래 백로그).

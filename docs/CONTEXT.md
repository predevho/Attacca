...
* 도메인 8개: MEMBER, VERIFIED-PERFORMER, FEED, PERFORMANCE, RECRUITMENT(구인/구직), CHAT, NOTICE, **IMPORT(2026-09-14 구현 완료)**
...
* IMPORT 운영 검증: `KOPIS_SERVICE_KEY` 발급과 `IMPORT_CONTACT` 결정 후 KOPIS 실제 호출, 대학 게시판 첫 페이지 수집, 어드민 승인·거절·포스터 저장 smoke test를 수행한다. 현재 자동 검증은 FE 493/493·lint 오류 0건·build 성공, BE JDK 21 `cleanTest test` `BUILD SUCCESSFUL`이다.

...
* 단계: BE 6개 도메인 전부 완료(인증/프로필/파일/DB + VERIFIED-PERFORMER + FEED + PERFORMANCE + RECRUITMENT + CHAT) + FE(인증/프로필/카카오) 완료. **다음은 FE 화면**(인증연주자/피드/공연/구인/채팅). CHAT은 `feat/chat-domain` 브랜치에 구현됨(미merge).
* 확정된 기술 스택
